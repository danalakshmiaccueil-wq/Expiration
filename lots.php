...
function createLot($pdo, $data) {
    ...
    $sql = "INSERT INTO lots (produit_id, numero_lot, date_expiration, date_fabrication, 
            quantite_initiale, quantite_actuelle, lieu_stockage, statut, date_creation, date_modification)
            VALUES (:produit_id, :numero_lot, :date_expiration, :date_fabrication,
            :quantite_initiale, :quantite_actuelle, :lieu_stockage, 'actif', NOW(), NOW())";
    ...
    $stmt->execute([
        ...
        ':date_fabrication' => $data['date_fabrication'] ?? date('Y-m-d'),
...
function markAsSold($pdo, $data) {
    ...
    // Ajouter la colonne date_solde si elle n'existe pas
    try {
        $pdo->exec("ALTER TABLE lots ADD COLUMN date_solde DATETIME NULL");
    ...
    $sql = "UPDATE lots 
            SET statut = 'soldé', 
                ...
                notes = CONCAT(COALESCE(notes, ''), '\nMarqué comme soldé le " . date('Y-m-d H:i:s') . "')
            WHERE id = :id";
...
function handlePut($pdo) {
    ...
    $stmt->execute([
        ...
        ':lieu_stockage' => $input['lieu_stockage'] ?? null,
